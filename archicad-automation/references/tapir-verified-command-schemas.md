...
- **Story numbering convention (verify EVERY time): the storey at reference level +/-0.00 (ground floor) gets index 0, basements negative (-1, -2, ...), upper floors positive (+1, +2, ...).** This is the standard convention in most European offices — treat it as the default unless the project dictates otherwise.
  ...
  - **Levels in the same call — version status (issue #573, FIXED upstream by #574, merged 2026-08-28):** on release <= 1.5.8 and all interim builds up to and including `6c4aafc`, the levels of a combined call come out WRONG (ladder shape partly correct but constantly offset, or applied relative; newer interim builds at least return a named error instead of silent success). Only from main >= `d90880a` (2026-08-28) do structure AND levels land correctly in one call — fix mechanics: pin the anchor first, then set the distances outwards (`SetElevation` moves only the story it names; `SetHeight` moves the side of the boundary further from the active story; the active story never moves). Until a release ships the fix, the **two-pass workaround** applies: create the structure with `index` first, then set the levels in a second pass — the FULL list per call, one target level per call, top-down; then verify via `GetStories`. Gate probe for the fix: one combined call on a scratch project — the readback must match exactly.
