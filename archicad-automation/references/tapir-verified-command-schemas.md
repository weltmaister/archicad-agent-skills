...
- **Enum properties: ALWAYS fetch the allowed values via `API.GetDetailsOfProperties`.** `GetAllPropertyNames` can report `String` where the real type is `singleEnum` (setting as string → `4008 "string instead of singleEnum"`); `GetDetailsOfProperties` returns `possibleEnumValues` with the EXACT spellings (spaces matter: `"T 30"`, not `"T30"`). Send as `{"type":"singleEnum","status":"normal","value":{"type":"displayValue","displayValue":"T 30"}}`. Rule of thumb: a semantically plausible-sounding property can carry the wrong values.
- WARNING — **Readback parsing for `singleEnum`:** the value sits in `propertyValue.value` as a **string**, NOT in `propertyValue.value.displayValue` — reading the expected path instead of the raw JSON has falsely reported a successful 25-value write as a failure.
- WARNING — **`DeleteAttributes` can silently delete nothing:** key `attributesToDelete` with `{attributeType, attributeId:{guid}}`, response `succeeded:true` — yet the attribute (live: a composite) remained and had to be removed by hand. Counter-check every attribute delete via `GetAttributesByType`.
- **Classification — two payload traps:** `SetClassificationsOfElements` needs `classificationSystemId` AND `classificationItemId` (item only → `succeeded:false` with an EMPTY error message); `GetClassificationsOfElements` expects `classificationSystemIds:[{classificationSystemId:{guid}}]` (bare `[{guid}]` fails). In the result, the tree name sits in the `id` field while `name` is empty.
- **`CreateComposites`: create and readback field names differ** — create `skins[]`: `buildingMaterialId`/`thickness`/`type:"Core"|"Finish"`/`framePen`; the readback names the same things `isCore`/`isFinish`/`skinType`/`framePenIndex`; `separators[]`: create `lineTypeId`+`linePen`, not `lineId`/`pen`. Using the readback as a create template yields `4002`.
- **GDL parameters: write only `gs_*` parameters.** `ac_*` prefixes are accepted with `success:true` and have no effect.
- **Check the active window BEFORE interpreting element counts:** Tapir only returns what belongs to the active database — a section window in front made all element counts read 0. `GetCurrentWindowType` is a mandatory step before any inventory.
- WARNING — **`GetElementsByType {elementType:"Stair"}` can reproducibly block in a real model** (>200 s timeout with 4 stairs, while Wall/Slab/Door/Zone answer in ms; SetDetails/GetPropertyValues on a SINGLE stair hung too). Approach: query stairs last, individually, with a short timeout; on hangs run the blockade diagnosis (see SKILL.md).
...
- `geometry` is one of two construction methods (schema `oneOf` — send exactly ONE):
  ...
  - **Zones as a tightness check of the wall model:** seed automatic zones from the room-text positions of the source plan; `-2130313215` means "no closed wall loop around the seed point" — every failure points at exactly one room with missing/leaky walls (live: 47 of 53 zones immediately; the 6 failures were precisely the unfinished rooms). The error pattern is a TEST RESULT, not a nuisance.
...
- **Use case beyond doors/windows:** slab voids that must be semantically visible downstream — a polygon hole in the slab is NOT an IfcOpening (see bim-element-modeling-rules.md). For plain geometric voids, `ModifySlabs` + `holes` is the lighter first path (WORKS under normal conditions — see ModifySlabs below); use `CreateOpenings` when the void must export as IfcOpening.
...
- Accepts `elementId`, `zCoordinate`, `thickness`, `structureType`, `buildingMaterialId`, `compositeId`, **`polygonOutline`**, `polygonArcs`, **`holes`**.
- **Geometry changes WORK under normal conditions** (CORRECTION of an earlier revision of this file): `holes` verified live 2026-08-28 (real project: test slab + a 35-point storey slab with 6 holes, three times) and 2026-08-30 (scratch, merged-main build: `holes` AND `polygonOutline` applied + read back). The item error `-2130312912 "Failed to modify slab geometry."` measured on 2026-08-20 is a **conditional** case (condition unresolved; candidates: hidden layer, renovation status, solid-element operations/trims, linked elements) — therefore ALWAYS read back, never trust `success` alone.
- WARNING — **A geometry modify replaces the WHOLE polygon memo:** sending `polygonOutline` WITHOUT `holes` DELETES the existing holes (live 2026-08-30: outline changed, hole gone). Always send outline and holes together.
- WARNING — **A hole outside the slab outline is silently dropped** — `success:true`, hole missing in the readback (live 2026-08-30).
- **`Holes2D` schema (the only effective shape — four plausible variants are accepted and ignored):** `"holes": [{"polygonCoordinates": [{"x":...,"y":...}, ...]}]`. The readback names the field `holes[].polygonOutline`. Applies to CreateSlabs/ModifySlabs/CreateRoofs/ModifyRoofs/CreateZones (manual).
- Name asymmetry with `CreateSlabs` (`polygonCoordinates` on create, `polygonOutline` on modify/readback): the wrong name yields `4002 additionalProperties` — do not misread as "the field does not exist".
...
- `GetDimensionData` — read witness-point data from existing chains. **Field names (live-verified):** the segment value is `dimensionValue` (NOT `measuredValue`); each witness also carries `coordinate`, `coordinate3D`, `dimensionPosition`, `witnessForm`, `witnessVal`, `baseElementId`. The FIRST witness has `dimensionValue: 0` — segment values start at the second.
- WARNING — **API-created witnesses are `witnessForm: "Fix"`** — height/opening dimensions that need associative points are not reachable through the API (known gap).
...
- **Story numbering convention (verify EVERY time): the storey at reference level +/-0.00 (ground floor) gets index 0, basements negative (-1, -2, ...), upper floors positive (+1, +2, ...).** This is the standard convention in most European offices — treat it as the default unless the project dictates otherwise.
  ...
  - **Levels in the same call — version status (issue #573, FIXED upstream by #574, merged 2026-08-28):** on release <= 1.5.8 and all interim builds up to and including `6c4aafc`, the levels of a combined call come out WRONG (ladder shape partly correct but constantly offset, or applied relative; newer interim builds at least return a named error instead of silent success). The merged fix (`d90880a`, pin the anchor first, then set the distances outwards) is **Archicad-version-dependent**: verified green by the maintainer on **AC29**, but demonstrably still failing on **AC28** (live 2026-08-30 on a merged-main build: the height pass is exact, but the `SetElevation` pin moves NO story while returning NoError, regardless of which story is active; the named error is reported correctly). Notably the same AC28 instance converged on 2026-08-20 through the OLD per-story `SetElevation` loop — so the call is not universally inert there; the context cause is unresolved and reported upstream on #574. **On AC28 the two-pass workaround therefore still applies even on the newest build; run the gate probe per instance before relying on one-call levels** (combined call on a scratch project — the readback must match exactly). Two-pass workaround: create the structure with `index` first, then set the levels in a second pass — the FULL list per call, one target level per call, top-down; then verify via `GetStories`. Gate probe for the fix: one combined call on a scratch project — the readback must match exactly.
...
- WARNING — **`CreateRoofs`: version-dependent!** On **all releases <= 1.5.5** the command is an unconditional stub — every item returns `-2130313215 "Multi-plane roof creation is not yet supported"`, NO payload works (issue #467); there, substitute roofs as slab/morph. **Fixed since release 1.5.6 (2026-07-31, #482). Verified live:**
  ...
  - `GetDetailsOfElements` for a Roof returns `{"error":"Not yet supported element type"}` (issue #569; same for `Opening`) — verify roofs via `Get3DBoundingBoxes` only.
  - WARNING — **Schema change on merged main (>= 2026-08-28):** `polygonCoordinates` is now REQUIRED even for the single-plane form — `pivotLine`+`angle` alone (valid on 1.5.6-1.5.9 interim builds) returns `4002 'required' on #/roofsData/0`. `pivotLine`+`polygonCoordinates` together is accepted (creates a single-plane roof). Probe per build before use.
  - **`ModifyRoofs` — roof geometry is effectively immutable after creation:** on builds up to late August 2026, `level` was accepted and silently ignored (`succeeded:true`, bbox unchanged); on merged main, single-plane roofs at least get an honest per-item error `-2130312313 "Only multi-plane roofs are supported."` (live 2026-08-30). Correction = delete + recreate.
